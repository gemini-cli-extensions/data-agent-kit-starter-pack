...
Knowledge Catalog (Dataplex) Lookup Context provides the richest metadata for
data assets. You MUST prioritize using it for all Google Cloud assets, even if
you already know their IDs.

-   **Public Datasets (Direct Inspection)**: If the requested asset belongs to
    the `bigquery-public-data` project, Knowledge Catalog Lookup Context will
    fail. You MUST skip Steps 3 and 4 and inspect the table directly using the
    `bq` CLI or BigQuery MCP tools instead.
-   **All Other Assets (Proceed to Step 3)**: For all other BigQuery, Cloud
    ...
    Knowledge Catalog (Dataplex) and obtain their full Entry Name.

## Step 3: Execute Discovery Search

You MUST use the Knowledge Catalog (Dataplex) search command to discover assets
and retrieve their full `projects/...` entry names. This step is required even
if you already know the asset's short ID (e.g., `my_dataset.my_table`), because
Step 4 strictly requires the full entry name.
...
#### Knowledge Catalog Search Syntax Reference
...
> [!TIP]
>
> Knowledge Catalog search results rely on metadata being ingested into the
...
>         inform the user. Ask for clarification, specifically the Knowledge
>         Catalog **full entry name** if known, or identifiers such as **project
>         ID**, **dataset ID**, or **instance ID** to help narrow the search.
>         Example: "I couldn't find any tables matching that description after
>         several attempts. If you know the Knowledge Catalog full entry name
>         (`projects/...`), please provide it. Otherwise, please provide any
>         identifiers you know, such as project, dataset, or instance name, to
>         help locate the asset."
> 2.  **Multiple Results:**
...
>     queries. If Knowledge Catalog fails to return the expected asset,
>     prioritize asking the user for the exact resource ID or using Fallback 2
>     (Native Tools).
...
-   **Cause**: The table belongs to a project (e.g., `bigquery-public-data`)
    that has not fully synchronized its metadata with the Knowledge Catalog
    Universal Catalog. While the entry appears in search, `context lookup` is
    unavailable.
...
-   **Stop Rule:** If the native tool (e.g., `bq show`) also returns "Not
    Found", STOP. Do not restart the discovery loop. Specifically ask the user
    to verify the **project ID** and **table ID**.
...
1.  **STOP.**
2.  State what you have tried (e.g., "I tried searching Knowledge Catalog with X
    and Y, and checked `bq show`").
