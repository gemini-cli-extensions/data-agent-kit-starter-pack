...
> parameter. The objective is automatically inferred from the classifier type: -
> `SparkXGBClassifier` → `binary:logistic` (inferred) - `SparkXGBRegressor` →
> `reg:squarederror` (inferred)
