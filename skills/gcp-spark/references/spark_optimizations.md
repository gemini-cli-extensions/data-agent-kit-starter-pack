...
-   **Before `.write.parquet()` or `.write.saveAsTable()`**: Repartition or
    coalesce to produce optimal 128MB–512MB file sizes. Adjust the target
    partition count (e.g. `4`) based on total data volume:
    ...
    # (Scale partition count based on total data volume to target ~128MB-512MB per file)
