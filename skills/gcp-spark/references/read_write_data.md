...
> [!IMPORTANT] **Flexible Spark Session Configuration Patterns**: Avoid
> hardcoding Apache Iceberg or BigLake REST Catalogs if metastores are not
> pre-configured. Use the appropriate session configuration strategy:
>
> 1.  **Standard Direct GCS Storage**: Write directly to
>     `gs://<bucket>/<table_name>` as Parquet/CSV.
> 2.  **BigLake REST Catalog**: Use when BigLake REST catalog properties are
>     explicitly provided.
> 3.  **Local PySpark Development**: Use local Hadoop catalog configurations.
>
> If an Iceberg catalog namespace does not exist, attempt to create it using
> `CREATE NAMESPACE IF NOT EXISTS` or ask the user for clarification before
> falling back.
