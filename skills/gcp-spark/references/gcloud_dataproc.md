...
### Lakehouse Catalog Zero-Config Discovery

If the user requests configuring a Dataproc environment with **simplified** or
**zero-config Lakehouse Iceberg catalog automatic discovery** (without manual
bootstrap configurations), you **MUST** apply the following specific properties
dynamically:

```
--properties="dataproc:dataproc.lakehouse.catalog.discovery.enabled=true,dataproc:dataproc.lakehouse.catalog.discovery.project=<PROJECT_ID>"
```

For Serverless batches, append these as Spark properties:

```
--properties="spark.dataproc.lakehouse.catalog.discovery.enabled=true,spark.dataproc.lakehouse.catalog.discovery.project=<PROJECT_ID>"
```
