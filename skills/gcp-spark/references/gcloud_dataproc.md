...
"clusters", "cluster jobs", or just "jobs", or when creating/provisioning a
cluster. **Do not** use this section if the user requests "serverless jobs",
"serverless batches", or "batches".

> [!IMPORTANT] Cluster-targeted workloads MUST submit jobs using `gcloud
> dataproc jobs submit pyspark ... --cluster=<CLUSTER_NAME>` and NEVER mix
> cluster provisioning with serverless `batches submit` (`gcloud dataproc
> batches submit`). If a Dataproc cluster is created or provisioned, any
> subsequent PySpark jobs MUST be submitted to that cluster via `gcloud dataproc
> jobs submit pyspark --cluster=<CLUSTER_NAME>`.

### Submitting jobs to clusters

> [!WARNING] This DOES NOT apply to executing **Python Notebooks (.ipynb)**.
> [!IMPORTANT] Refer to this guide when executing **PySpark Script (.py)** on
> Dataproc Clusters.

Prefer MCP if available. If using gcloud, use this command template:

```
gcloud dataproc jobs submit pyspark <SCRIPT_PATH.py> \
    --cluster=<CLUSTER_NAME> \
    --project=<PROJECT_ID> \
    --region=<GCP_REGION>
```

Augment with required arguments and properties (e.g. `--properties=...`) as
needed.
...
Serverless. **NEVER** use serverless `batches submit` when a Dataproc cluster is
...
> on Serverless Spark. [!IMPORTANT] Do NOT use `batches submit` if running on or
> creating a Dataproc Cluster. Cluster workloads MUST submit jobs using `gcloud
> dataproc jobs submit pyspark ... --cluster=<CLUSTER_NAME>`. NEVER mix cluster
> provisioning with serverless batch submission.
