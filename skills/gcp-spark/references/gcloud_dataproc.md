...
-   **PROHIBITED**: Do NOT write custom bash `while` loops or Python scripts
    with `time.sleep()` to poll the batch status repeatedly. This wastes tokens
    and system resources.
-   **Waiting for completion**: The standard `gcloud dataproc batches submit`
    command blocks until completion. Rely on the environment's background task
    management (you will be notified when it completes). If you must wait for an
    already-running batch, use `gcloud dataproc batches wait <BATCH_ID> \
    --project=<PROJECT_ID> --region=<REGION>`.
-   **Checking status once**: To inspect a batch's terminal state or current
    status a single time without blocking, use `gcloud dataproc batches describe
    \ <BATCH_ID> --project=<PROJECT_ID> --region=<REGION>`. ### Connector
