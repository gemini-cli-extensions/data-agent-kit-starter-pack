...
2.  **Verify storage accessibility and handle GCS errors**: Verify access or
    existence using `gcloud storage ls gs://<path-to-dataset>`. If accessing,
    reading, or saving/uploading notebooks or artifacts to a GCS destination
    path fails with a storage error e.g., permission errors like `403
    Forbidden`/`Forbidden`/`PermissionDenied`, or location errors like `404 Not
    Found`/`NotFound`/`FileNotFoundException`, report the error immediately.
    Either (1) ask the user what to do next, or (2) if asked to execute a
    notebook, retain the local notebook with all executed cell outputs, report a
    summary of the verified execution results along with the destination
    permission error, and recommend next steps to resolve IAM access. Do NOT
    scan all buckets for alternative fallback datasets, and do NOT enter retry
    loops, change project configurations, or attempt unauthorized bucket
    overrides when encountering GCS errors.
...
5.  **Compile and execute notebook code**: For notebooks convert them to python
    ...
    your-script.py`. The same can be done for pyspark source code. When
    requested to execute or evaluate a notebook, use `execute_cell` or `jupyter
    nbconvert --to notebook --execute --inplace your-notebook.ipynb` and verify
    cell outputs using `get_cell_outputs`. If MCP cell execution reports
    unstarted kernels, do NOT attempt to guess or call non-existent connection
    tools (e.g., `colab__connect`); execute the notebook directly with `jupyter
    nbconvert --to notebook --execute --inplace your-notebook.ipynb`.
...
-   [ ] **Notebook Execution & Tooling** In automated environments, if MCP cell
    execution reports unstarted kernels, execute the notebook with `jupyter
    nbconvert --to notebook --execute --inplace <notebook.ipynb>`. NEVER
    hallucinate non-existent tools like `colab__connect`.
