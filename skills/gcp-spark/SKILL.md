...
-   [ ] **Safe cleanup of temporary environments (`spark_env`)** When cleaning
    up local virtual environments or directories, use `python3 -c "import
    shutil; shutil.rmtree('spark_env', ignore_errors=True)"` or `rm -r
    spark_env` instead of `rm -rf` to prevent shell confirmation prompts in
