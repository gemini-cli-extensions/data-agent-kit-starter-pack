...
7.  **Verify output materialization**: **ALWAYS** perform explicit
    post-execution verification on output destinations (e.g., run `gcloud
    storage ls gs://<output-path>` for GCS exports, or query destination tables
    in BigQuery/Spanner) to confirm successful write.
