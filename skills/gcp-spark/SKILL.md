...
1.  **Understand schemas**: **ALWAYS** use `@skill:discovering-gcp-data-assets`
    ...
    any code. Do NOT guess column names. In notebooks, always inspect schemas
    with `df.printSchema()` and verify missing/null values before transforming.
    Unless explicitly specified, assume that the assets are located in the same
    project. Avoid scanning for assets across other projects as it can take a
    long time. If an expected dataset or table does not exist, use
    `@skill:discovering-gcp-data-assets` to discover all similar tables in the
...
3.  **Generate spark code**:
    ...
    *   **Data Integrity**: **NEVER** substitute real data sources with mock,
        dummy, or random data to bypass errors.
    ...
    *   **Insights & Interpretation**: Include markdown cells that interpret
        numerical outputs, patterns, and trends rather than merely describing
        code steps.
...
-   [ ] **No mock data substitution** NEVER generate mock datasets or fake
    output cells when encountering execution errors.
