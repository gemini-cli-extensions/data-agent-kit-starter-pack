...
description: 'Primary entry point for building, managing, and orchestrating data pipelines
  ...
  Beam), Dataform, Spark (Dataproc Serverless) or orchestration pipeline using Cloud
  Composer. Clarify requirements and resolve ambiguity for creating, updating and
...
----------------- | -----------------------------------------------------------
**Dataflow**      | `.java` files containing `import org.apache.beam`, `.py` files containing `import apache_beam`
...
-   If an existing pipeline is detected via an unambiguous indicator (e.g.,
    ...
    it, you MUST **proceed directly** using that pipeline's skill — you MUST
...
| ----------------- | ------------------------------------ | ----------------- |
| **dbt**           | SQL-first teams; modular models with | `dbt-bigquery`    |
:                   : built-in tests & docs; all           :                   :
:                   : transforms run inside BigQuery       :                   :
| **Dataflow**      | Streaming pipelines; Apache Beam;    | `gcp-dataflow`    |
:                   : Unified stream and batch processing; :                   :
:                   : High-throughput Pubsub integration;  :                   :
:                   : ML Preprocessing and Inference       :                   :
:                   : at scale; Advanced observability;    :                   :
:                   : Serverless data processing           :                   :
| **Dataform**      | Google-native ELT; GCP Console       | `dataform-bigquery` |
:                   : integration; SQLX/JS for complex     :                   :
:                   : dependency management                :                   :
| **Spark (Dataproc | Large-scale data;                    | `gcp-spark`  |
: Serverless)**     : PySpark/Java/Scala; ML               :                   :
:                   : preprocessing; Iceberg/BigLake       :                   :
| **Other**         | Data Fusion, or generic Python       | —                 |
:                   : — proceed with general GCP           :                   :
:                   : assistance                           :                   :

**Orchestration** — separate concern,used to GCP Data Pipeline Orchestration -
deploy and coordinate multiple pipelines already in the repo:

| Option             | Best For                 | Skill                        |
| ------------------ | ------------------------ | ---------------------------- |
| **Cloud Composer** | GCP Data Pipeline        | `gcp-pipeline-orchestration` |
:                    : Orchestration            :                              :
:                    : deploy/schedule existing :                              :
:                    : pipelines(dbt + Spark,   :                              :
:                    : etc.). as a unified      :                              :
:                    : workflow                 :                              :
...
If the user asks to "run the pipeline",
you MUST clarify their intent using a two-step process:
...
1.  **Clarify Method:** If an orchestration pipeline exists, use
