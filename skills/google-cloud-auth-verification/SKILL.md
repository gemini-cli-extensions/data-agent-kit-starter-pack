...
description: >-
  ...
  MCP server tools, Application Default Credentials (ADC), gcloud CLI, Spark, Dataproc, BigQuery, GCS, and notebook runtimes.
...
>
> 1.  **Check Active GCP Authentication**: Verify active GCP authentication via
>     available tools (active MCP server connection, `gcloud auth list`, or
>     Application Default Credentials).
> 2.  **Verify `gcloud` Credentials**: If shell-based commands or cloud storage
>     paths (`gs://...`) are required, check `gcloud auth list` or Application
>     Default Credentials (ADC).
> 3.  **Distinguish Authentication vs. IAM Permissions**:
>     -   If `gcloud auth list` returns `No credentialed accounts` or WebAuthn
>         re-auth errors, **HARD STOP** immediately and instruct the user to run
...
1.  **gcloud/bq CLI**:
    ...
    -   `Reauthentication required. Insert and touch security key` (or exit
        status 11 in subshells).
...
*   If `gcloud` subshell fails with `Reauthentication required. Insert and touch
    security key` (WebAuthn error), **HARD STOP** immediately and instruct the
    user: *"gcloud requires security key re-authentication. Please run `gcloud
    auth login` interactively in your terminal."*
